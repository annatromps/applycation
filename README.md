...
- **Automatic posting resolution**: any job without a real posting URL/description (added manually, imported from a board this app doesn't scrape, etc.) automatically gets a lookup attempt against the company's own job board, in two tiers (`server/postingResolver.js`):
  - **Tier 1 — free ATS APIs**: checks Greenhouse, Lever, Ashby, and Recruitee in parallel, using each one's free, public, unauthenticated JSON API (the same kind `server/sources/` already reads from for regular discovery — see `server/atsLookup.js`), matched by title + company name. Several slug variants are tried per company (the full name, with corporate suffixes like "Inc"/"Labs"/"Technologies" stripped, and just the first word for multi-word names), since the guessed slug rarely equals the literal company name. No scraping, no per-job manual web fetches, nothing to approve. Always tried first, and free.
  - **Tier 2 — AI web search (optional)**: if tier 1 finds nothing *and* an Anthropic or Gemini provider + API key are configured under Settings → Advanced, falls back to a real, grounded web search to find the actual result — this covers everything tier 1 structurally can't reach: Workday, iCIMS, a plain custom careers page, whatever. For Anthropic this is Claude's hosted, server-executed web search tool; for Gemini it's Google's own search grounding via the Gemini Interactions API (`server/ai/client.js`'s `callGeminiWithSearch`) — Gemini's response hands back the actual citation URLs its search found, and this app cross-checks the model's claimed answer against those before trusting it, an extra guard on top of the same rule both providers are instructed to follow: never guess or construct a URL from a pattern, and say so plainly (`{"found": false}`) rather than return a low-confidence guess — same "never fabricate a link" rule the rest of the app follows. Both providers bill grounded search separately from normal usage (Gemini specifically needs a billing-enabled key for this feature, unlike the rest of its usage which stays on the plain free tier — though at this app's low, capped volume it should land inside Google's free daily grounding allowance and cost $0 in practice), so the startup backfill pass caps how many tier-2 lookups it'll spend per run via "Max AI web-search posting lookups per startup pass" in Settings → Advanced (default 10) — the on-demand single-job "Find posting" button and manual-add aren't capped, since those are one job at a time. Skipped entirely with no Anthropic/Gemini provider configured, or with Groq (no search capability wired in here). **Note on the Gemini path**: it's built from Google's published API reference, not verified against a live key (no outbound network access from the build sandbox) — if it's not finding anything, check the Railway logs for a `[aiPostingSearch] Gemini lookup failed` line, which includes the raw HTTP status/response from Google to make it quick to spot and fix a wrong field/endpoint name.

  Runs automatically when such a job is added and, for anything still missing a URL, once more at every server startup (`server/index.js`'s backfill). You can also retry it on demand from a job's detail view ("Try to find the real posting automatically"), or in bulk from the Tracker's "Find missing postings" button, which runs it across every job in view that's still missing a link and reports back how many it found. Even with both tiers this is best-effort: a company off all four ATS platforms with no Anthropic provider configured, or one tier 2 genuinely can't find a confident match for, is left exactly as it was, and the detail view's "Posting details" section lets you paste the real URL/description/location/salary in yourself instead.
- **Tracker**: every job moves through a status pipeline (discovered → reviewing → approved → materials ready → submitted → interviewing → offer/rejected/withdrawn), with full history, notes, and dates, filterable in the Tracker tab. Each row has a ★ favourite toggle (click to star/unstar, no need to open the row) and a 📝 notes button (a quick prompt for a short note, independent of the fuller notes field in the detail view below — either one edits the same `notes` field on the job, whichever's more convenient in the moment); a "★ Favourites only" checkbox above the table filters down to just your starred jobs. Click any row (not the star/notes buttons) to open its full detail view — ratings breakdown, review questions, notes, status history, generated materials, and an editable "Posting details" section (URL, description, location, salary — auto-rescored the moment you save a change, see the "Always automatic, no rescore button" note above). Company name is the prominent element everywhere a job is listed (Dashboard, Review Queue, Tracker, detail view), each shown next to a best-effort logo (Clearbit's free logo API, falling back to Google's favicon service, falling back to a plain initial-letter avatar when neither has a match — never blocks on this), with a direct link to the actual posting wherever a URL is on file. The Dashboard's "Awaiting review" stat card jumps to the Review Queue, and "Applied"/"Interviewing" jump to the Tracker pre-filtered to every status that counts towards that stat (not just one exact status).
- **Job-alert email import (optional)**: none of the big job sites (LinkedIn, Indeed, and others) offer a free public jobs API, and scraping any of them breaks their terms of service, so this app never fetches anything from a job site directly — full stop. Instead, under Settings → Advanced → "Job-alert email import", you can connect an email inbox (IMAP, e.g. a Gmail account with an [App password](https://myaccount.google.com/apppasswords)) that receives job-alert emails, one inbox and app password covering as many job sites as you add: the sender filter is a comma-separated list of addresses/domains (e.g. `linkedin.com, indeed.com, welcometothejungle.com, wellfound.com`) — a whole domain is a deliberate catch-all for every kind of alert email that site sends, since exact sender addresses aren't always known and change over time. Each discovery cycle, if enabled, the app reads any new matching emails from that inbox from the last 60 days — dedup is by the email's own Message-ID (persisted, so already-imported mail isn't reprocessed or re-billed for AI extraction), not by whether it's marked read, so a backlog of job-alert emails you'd already opened before connecting the app gets picked up too, not just brand-new arrivals. It still marks each one read as a courtesy (never deleting), but that flag no longer matters for dedup. It cheaply skips anything with no actual job-posting link or job-ish subject line in it (connection requests, InMail, marketing, etc. — so a broad sender filter doesn't waste AI calls), extracts the individual job listings mentioned from what's left (AI-assisted when a provider is configured, since every job site's email markup shifts over time and this generalizes to a site the app has never seen before; a plain regex sweep recognizing several known job-board URL shapes plus a generic `/jobs/`-style fallback otherwise), and tries to resolve each one to the employer's own posting using the same free public ATS APIs (`server/atsLookup.js`) the regular sources and automatic posting resolution use — giving you a direct company link and real description instead of the original digest-email link, when a confident match exists. Falls back to that original link, untouched, when it can't find one — never fabricates a URL or description. These jobs flow through the exact same scoring/materials/review-questions pipeline as anything else, tagged `source: "email-digest"`. A health indicator (green/red dot + last result) sits right under the "Import job-alert emails" checkbox, updated automatically after every discovery cycle and on-demand via its "Test connection" button — so a bad app password or connection problem is visible at a glance instead of silently doing nothing. Not sure which domains to add? The "🔍 Scan my inbox for job sites" button does a read-only scan of recent mail (envelopes only — sender + subject, never the body) for senders that look like job alerts you're not already watching, and lets you confirm or dismiss each one before it's added to the sender-filter field. See `server/email/`.
- **"Run discovery now"**: the Dashboard button that kicks off `runDiscoveryCycle()` on demand — job-board sources, email-digest import, posting-lookup resolution, and scoring all run as one request, so the button's label cycles through short status lines (e.g. "📬 Scanning your email digests…") while it's in flight, giving a sense of what's happening even though there's no real step-by-step progress being streamed back. A "0 new matches" result opens a small breakdown instead of a bare alert — how many raw results each job-board source and the email digest returned, how many were already-known, how many got ruled out by a dealbreaker, and how many scored below your match threshold — so "0" is diagnosable instead of a black box. The single most common cause: **discovery does nothing at all — no sources queried, no email checked — if there's no active Criteria Profile**, since everything is scored against one; that case gets its own direct call-out with a link to the Me tab rather than a confusing zeroed-out breakdown.
- **AI provider connection test**: next to the AI provider fields in Settings → Advanced, a "Test connection" button sends one small real request to verify the provider/key/model combination actually works, with the same green/red health indicator pattern as the email import above — catches a bad or mistyped key immediately instead of it silently failing the next time scoring, drafting, or the posting-search fallback tries to use it. Never exercises the separately-billed web-search capability, just plain connectivity.
...
server/
  ...
  email/          optional job-alert email import (LinkedIn, Indeed, or any other job site) — inbox.js (IMAP polling, multi-sender), parseDigest.js (entry extraction, AI-assisted + rule-based), resolveApplyLink.js (best-effort ATS resolution via atsLookup.js), digestImport.js (ties them together into the discovery pipeline)
