...
- **Scoring**: every discovered job is scored 0-100 against your criteria profile — location, remote locations, visa needs, languages, title/role keywords, role type, level, minimum salary, role priorities, dealbreakers, favourite/hidden industries, favourite/hidden technologies, company size, and followed companies — with a plain-English reason list for every point added or subtracted. Only jobs above your configured threshold, and with no dealbreaker hit, are surfaced.
- **AI preferences (optional)**: each criteria profile also has a free-text box for describing fit in your own words (culture, pace, red flags, anything the structured fields can't capture). If you've set an Anthropic API key in Settings, the strongest rule-based matches each cycle get a second pass where that text is sent to Claude alongside the posting for a qualitative score, blended with the rule-based one. Bounded by a configurable "max AI-scored jobs per cycle" so cost stays predictable; skipped entirely with no key set.
- **Review queue**: new matches wait for your approval before anything else happens. Approve → materials get generated automatically. Dismiss → it's dropped.
...
- **Materials generation**: builds a tailored CV (.docx) and cover letter (.docx) from your profile data for every job you approve, stored against that job and downloadable from its detail view. The CV reorders each role's bullets to surface the most relevant ones first, without inventing or dropping anything. The cover letter picks your most relevant "talking points" for that posting; if you provide an Anthropic API key in Settings it drafts more naturally with Claude, otherwise it assembles a straightforward template — either way it's run through your house rules (e.g. no em dashes) before saving.
...
server/
  ...
  docgen/         CV + cover letter .docx generation (as Buffers), CV text extraction (PDF/DOCX), AI profile import
