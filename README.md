...
- **Manually add a job**: use the "+ Add job manually" button on the Tracker page (title/company required; location, URL, status, and notes optional), or `POST /api/jobs` directly with the same fields. Hand-added jobs skip scoring entirely — no criteria profile exists to score them against — so they show "–/10" rather than a fabricated number, and default to "reviewing" status.
...
server/
  ...
  docgen/         CV + cover letter .docx generation (as Buffers), materials.js ties both together per job (+ tailoringSummary.js's plain-English "what changed" note), CV text extraction (PDF/DOCX), AI profile import
