...
- **"Run discovery now"**: the Dashboard button that kicks off `runDiscoveryCycle()` on demand — job-board sources, email-digest import, posting-lookup resolution, and scoring all run as one request, so the button's label cycles through short status lines (e.g. "📬 Scanning your email digests…") while it's in flight, giving a sense of what's happening even though there's no real step-by-step progress being streamed back.
